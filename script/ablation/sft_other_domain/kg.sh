...
do
    ...
    do
        ~/.conda/envs/OFA/bin/python finetune.py --pt_data default --sft_data $sft_data \
        --exp_group base-model-results --use_params --no_split\
        --setting $setting --dataset $dataset
    done
done

for dataset in cora citeseer pubmed arxiv23 dblp arxiv bookhis bookchild elecomp elephoto sportsfit amazonratings chemhiv bbbp bace toxcast cyp450 tox21 muv products chempcba
do
    for setting in base
    do
        ~/.conda/envs/OFA/bin/python finetune.py --pt_data default --sft_data $sft_data \
